...
inline float qx_max_f3(float *a)
{
...
inline void qx_image_dot_product(double *out, float *a, unsigned char *b, int len)
{
	for (int i = 0; i < len; i++)
		*out++ = double(*a++) * double(*b++);
}
inline void qx_image_dot_product(double *out, double *a, double *b, int len)
{
	for (int i = 0; i < len; i++)
		*out++ = (*a++) * (*b++);
}
...
inline void image_zero(float **in, int h, int w, float zero = 0.0) { memset(in[0], zero, sizeof(float) * h * w); }
...
inline int qx_square_difference_u3(unsigned char *a, unsigned char *b)
{
	int d1, d2, d3;
	d1 = (*a++) - (*b++);
	d2 = (*a++) - (*b++);
	d3 = (*a++) - (*b++);
	return (int(d1 * d1 + d2 * d2 + d3 * d3));
}
...
{
	double *table_color, *color_table_x;
	int y;
...
{
	double *color_table_x;
	int y;
	color_table_x = &table_color[0];
	for (y = 0; y < len; y++)
		(*color_table_x++) = exp(-double(y * y) / (2 * dist_color * dist_color));
}
...
{
	int x0, xt, y0, yt;
	double dx, dy, dx1, dy1, d00, d0t, dt0, dtt;
	x0 = int(x);
	xt = min(x0 + 1, w - 1);
	y0 = int(y);
	yt = min(y0 + 1, h - 1);
...
inline double ***qx_allocd_3(int n, int r, int c, int padding = QX_DEF_PADDING)
{
	...
	if (a == NULL)
	{
		printf("qx_allocd_3() fail, Memory is too huge, fail.\n");
		getchar();
		exit(0);
	}
...
inline unsigned char **qx_allocu(int r, int c, int padding = QX_DEF_PADDING)
{
	...
	if (a == NULL)
	{
		printf("qx_allocu() fail, Memory is too huge, fail.\n");
		getchar();
		exit(0);
	}
...
inline float **qx_allocf(int r, int c, int padding = QX_DEF_PADDING)
{
	...
	if (a == NULL)
	{
		printf("qx_allocf() fail, Memory is too huge, fail.\n");
		getchar();
		exit(0);
	}
...
inline float ***qx_allocf_3(int n, int r, int c, int padding = QX_DEF_PADDING)
{
	...
	if (a == NULL)
	{
		printf("qx_allocf_3() fail, Memory is too huge, fail.\n");
		getchar();
		exit(0);
	}
...
inline int **qx_alloci(int r, int c, int padding = QX_DEF_PADDING)
{
	...
	if (a == NULL)
	{
		printf("qx_alloci() fail, Memory is too huge, fail.\n");
		getchar();
		exit(0);
	}
...
inline unsigned char ****qx_allocu_4(int t, int n, int r, int c, int padding = QX_DEF_PADDING)
{
	...
	if (a == NULL)
	{
		printf("qx_allocu_4() fail, Memory is too huge, fail.\n");
		getchar();
		exit(0);
	}
...
inline void qx_memcpy_u3(unsigned char a[3], unsigned char b[3])
{
	*a++ = *b++;
	*a++ = *b++;
	*a++ = *b++;
